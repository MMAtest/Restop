...
def run_test():
    print("🚀 DÉMARRAGE DU CRASH TEST OCR (CORRIGÉ) - FICHIERS RÉELS")
    print("-------------------------------------------------------")
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                if 'document_id' in response_json:
                    ...
                    print(f"❌ Pas d'ID trouvé dans la réponse: {response_json.keys()}")
                    continue
                ...
                if len(analysis.get('items', [])) > 0:
                    print("   --- Aperçu des produits détectés ---")
                    for item in analysis.get('items', [])[:5]: # Top 5
                    ...
                    # Si aucun produit, on affiche un extrait du texte brut pour diagnostic
                    if 'texte_extrait' in response_json:
                        print(f"   📝 Extrait texte brut: {response_json['texte_extrait'][:100]}...")
