...
const RoleBasedDashboard = ({ user, sessionId, onNavigateToPage }) => {
  ...
  return (
    <div style={{marginBottom: '20px'}}>
      ...
      {/* Accès rapides aux pages autorisées selon le rôle */}
      {user.role !== 'super_admin' && (
        <div style={{
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px'}}>
            {(() => {
              // Définir les accès selon le rôle
              let allowedPages = [];
              
              switch (user.role) {
                case 'chef_cuisine':
                  allowedPages = [
                    { icon: '📦', label: 'Stocks', page: 'stocks' },
                    { icon: '🥘', label: 'Production', page: 'production' },
                    { icon: '🛒', label: 'Commandes', page: 'orders' }
                  ];
                  break;
                case 'caissier':
                  allowedPages = [
                    { icon: '📦', label: 'Stocks', page: 'stocks' },
                    { icon: '🔪', label: 'Préparations', page: 'stocks' },
                    { icon: '🛒', label: 'Suivi', page: 'orders' }
                  ];
                  break;
                case 'barman':
                  allowedPages = [
                    { icon: '🍹', label: 'Produits Bar', page: 'stocks' }
                  ];
                  break;
              ...
              return allowedPages.map((access, index) => (
                <button
                  key={index}
                  onClick={() => {
                    if (onNavigateToPage) {
                      onNavigateToPage(access.page);
                    }
                  }}
                  style={{
                    padding: '12px 8px',
                ...
                >
                  <div style={{fontSize: '20px', marginBottom: '4px'}}>{access.icon}</div>
